...
def test_delete_project():
    ...
    res = client.post(
        "/project/delete", json={"project_code": project_code}, cookies=cookie
    )
    res = db.list_project_by_owner(user_code)
    assert len(res) == 0
...
def test_change_project():
    ...
    res = client.get(
        f"/project/query?project_code={project_code}", cookies=cookie
    )
...
def test_add_project():
    ...
    res = client.post(
        "/project/add",
        json={"project_code": project_code, "new_member": "test"},
        cookies=cookie,
    )
    assert res.json()["success"] is True
...
def test_list_projects_by_condition():
    ...
    res = client.get(
        f"/project/list_projects?members={user_code}", cookies=cookie
    )
    assert res.json()["success"] is True
...
def test_count_projects_by_condition():
    ...
    res = client.get(
        f"/project/count_projects?members={user_code}", cookies=cookie
    )
    assert res.json()["success"] is True
...
def test_stat_issue_state():
    ...
    res = client.get(
        f'/project/stat_status?project_code={project_code}&'
...
def test_stat_issue_date():
    ...
    before_date = datetime.now()
    res = client.get(
        f'/project/stat_date?project_code={project_code}&'
        f'after_date={after_date.strftime("%Y-%m-%d")}&'
        f'before_date={before_date.strftime("%Y-%m-%d")}',
        cookies=cookie,
    )
