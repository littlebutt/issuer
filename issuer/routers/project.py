...
async def new_project(project: "ProjectReq",
    ...
    # 把自己添加到项目中去
    project = db.list_project_by_owner(_user.user_code)[-1]
...
async def delete_project(project: "ProjectReq",
    ...
    if project_do.issues is not None and len(project_do.issues) != 0:
...
@router.get('/participants', response_model=List[ProjectRes])
