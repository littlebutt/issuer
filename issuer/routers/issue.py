...
@router.post("/new")
async def new_issue(
    issue: "IssueReq", current_user: Annotated[str | None, Cookie()] = None
):
    """
    新增议题
...
async def change_issue(
    ...
    issue_do.description = (
        issue.description
        if issue.description is not None
        else issue_do.description
    )
...
@router.get(
    "/list_issues", response_model=Dict[str, bool | str | List[IssueRes]]
)
async def list_issues_by_condition(
...
async def count_issues_by_condition(
    issue_code: Optional[str] = None,
    project_code: Optional[str] = None,
    owner: Optional[str] = None,
    status: Optional[str] = None,
    ...
    if tags is not None:
        tags = tags.split(",")
    res = db.count_issues_by_condition(
